...
export default function AddressesPage() {
  ...
  // ✅ Hide the create form once user already has an address (but allow edit form)
  const hasAddress = (addresses?.length ?? 0) > 0;
  const showForm = !hasAddress || editId != null;
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      // ✅ UI bypass protection: don't allow creating more than one
      if (hasAddress) {
        showToast("You already have an address. Please edit the existing one.");
        return;
      }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <UserNavBar
        ...
        cartTotalLabel={
          loggedIn && !cartLoading && !cartError ? kes(cartTotal) : "KSh0.00"
        }
            ...
            <section className="lg:col-span-1">
              <div className="rounded-3xl border border-gray-200 bg-white p-5 shadow-sm">
                ...
                <div className="mt-1 text-xs text-gray-600">
                  You already have a saved address. To change it, click{" "}
                  <b>Edit</b> on the address.
                </div>
              </div>
            </section>
          )}
              ...
              {isLoading ? (
                <div className="space-y-3">
                  {Array.from({ length: 3 }).map((_, i) => (
                    <div
                      key={i}
                      className="animate-pulse rounded-3xl border border-gray-200 p-4"
                    >
