...
type Props = {
  ...
  // ✅ accept number or string from API
  price: string | number;

  // ✅ NEW: range upper bound from API (snake_case)
  max_price?: number | string | null;

  inStock?: boolean;

  // still allowed for wishlist storage (but NOT displayed on card)
  categoryName?: string;
...
type WishlistItem = {
  ...
  // ✅ store rendered string so wishlist stays stable
  price: string;
...
function toNumber(v: unknown): number | null {
  ...
  if (typeof v === "string") {
    ...
    // ✅ allow strings like "Sh 580" or "Ksh 580" or "580"
...
function formatPriceRange(minVal: unknown, maxVal: unknown): string {
  ...
  // if price is a non-numeric string (already formatted), show as-is
      ...
      : [
          {
            ...
            price: displayPrice,
            inStock,
            categoryName, // ✅ stored but not shown on card
            savedAt: new Date().toISOString(),
          },
          ...items,
        ];
